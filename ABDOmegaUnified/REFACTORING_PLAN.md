...
> **Estado global:** Fases 1-4 completadas (ModuleRenderer, ModuleManager, ModulePatchbayMatrix, editor web); Fases 5.1, 5.2, 5.3 y 5.4 (C++) completadas — pendientes 5.5 y 5.6; **Fase 6 completada** (bundle regenerado + **UI embebida en el exe standalone**). **Bugfix (2026-07-31):** `ParameterMetadataRegistry` se auto-inicializa en su constructor (ver log). **CMake DRY (2026-08-01):** argumentos de stage/deps de test/define JUCE en variables únicas — `OMEGA_UI_STAGE_ARGS`, `OMEGA_TEST_COMMON_DEPS`, `OMEGA_JUCE_SETTINGS_DEFINE` (ver log). **Bugfix build_wasm.bat (2026-08-01):** el script no arrancaba en cmd.exe (`No se esperaba . en este momento`) — paréntesis sin escapar `(emsdk)` dentro del bloque `if (...)` multilínea + finales LF. Fix `^(emsdk^)` + CRLF; pipeline WASM validado con Emscripten 6.0.4 (ver log, changelog #714). **Auditoría paréntesis .bat (2026-08-01):** los 15 `.bat` del proyecto revisados contra el patrón del bug — **0 archivos afectados** (el único infractor era build_wasm.bat, ya corregido); escáner `scripts/check_bat_parens.mjs` conservado como diagnóstico reutilizable, calibrado contra cmd.exe real (ver log, changelog #715). **Bat hardening (2026-08-01):** escáner integrado como **paso 0 (fail-fast)** en `build_auto.bat` (#716) y comentarios `::` dentro de bloques migrados a `REM` en build_plugins.bat/lint.bat (#717). **Guardia launchers (2026-08-01):** `scripts/check_bat_parens.cmd` (helper `call`-able con modo híbrido default y `/strict`) integrado como guardia pre-arranque en `start.bat`, `start_synth.bat` y `web/start.bat` (#718) — los launchers que no pasan por build_auto.bat. Comportamiento híbrido: sin node/escáner → warn + continúa; con issues → aborta (exit 1). **DRY:** `build_auto.bat` (#716) ahora delega en el mismo helper con `/strict`, eliminando su bloque inline duplicado — un solo punto de invocación del escáner. **CRLF (2026-08-01):** los 14 `.bat`/`.cmd` restantes en LF normalizados a CRLF (convención de build_wasm.bat/#714) y `.gitattributes` raíz añadido con `*.bat/*.cmd text eol=crlf` — con autocrlf=true el repo almacena LF y el checkout produce CRLF (#719). **Escáner `::` en bloques (2026-08-01):** `check_bat_parens.mjs` extendido para flaggear `::` dentro de bloques multilínea (automatiza la convención REM de #717); fixtures canónicos en `scripts/_bat_test/` (excluidos del scan) (#720).
...
| 2026-08-01 | Bugfix build_wasm.bat | ✅ | **El script no se podía ejecutar bajo cmd.exe** — fallaba con `No se esperaba . en este momento` justo tras `[1/4]`. Causa raíz aislada por bisectiva empírica (bats mínimos A-F): **paréntesis sin escapar `(emsdk)` dentro del bloque `if (...)` multilínea** — en cmd.exe el `)` de `(emsdk)` cierra el bloque prematuramente y el `.` sobrante rompe el parser (no era CRLF ni `>nul 2>nul`). Fix: escape canónico `^(emsdk^)` + normalización de finales a CRLF (el archivo era LF-only, 0 CR / 37 LF). **Validado definitivamente:** `build_wasm.bat` completo `[1/4]`→`[4/4]` exit 0 con emsdk (`/c/emsdk`, Emscripten 6.0.4); los 3 `.wasm` regenerados hoy (2026-08-01) con cabecera `\0asm` válida (864/1718/3837 B — menores que la build de mayo 1630/2610/4981 por la optimización de 6.x, sin errores de SIDE_MODULE); `engine/bindings/wasm_compat.h` presente. Changelog #714 |
| 2026-08-01 | Auditoría paréntesis .bat | ✅ | **Auditoría preventiva del bug de build_wasm.bat sobre los 15 `.bat` del proyecto** (build_auto, sync_omega_ui, build_plugins, start, start_synth, web/start, web/omega-audit, web/scripts/*, host/*). Método: escáner propio `scripts/check_bat_parens.mjs` — stack de paréntesis consciente de bloques multilínea (omite regiones `%...%` y escapes `^x`; flaggea **cualquier `)` sin escapar dentro de un bloque multilínea con contenido sobrante**; excluye patrones legítimos `) else (` y `) do (`) — calibrado contra cmd.exe real con 5 bats mínimos: `(emsdk).` en bloque FALLA (`No se esperaba .`), `^(emsdk^)` OK, `%ProgramFiles(x86)%` en bloque else OK (regiones `%var%` exentas), paréntesis en echo top-level OK, `for /f ... in (...) do (` en bloque if OK (exit 0). **Resultado: 0 issues en los 15 `.bat` reales** — el único infractor era build_wasm.bat (ya corregido, #714); no se aplicaron fixes preventivos porque no existen otros patrones peligrosos. Escáner conservado como diagnóstico reutilizable. Nota adyacente: `::` dentro de bloques (build_plugins.bat) es un footgun distinto de cmd.exe — fuera de scope, candidato de follow-up. Changelog #715 |
| 2026-08-01 | Integración escáner en build_auto.bat | ✅ | Paso 0 (fail-fast) en `host/build_auto.bat`: verifica `node` + ejecuta `scripts/check_bat_parens.mjs`; aborta el build si algún `.bat` reintroduce paréntesis sin escapar en bloques (bug #714). Probado end-to-end con cmd.exe real: repo limpio → `[OK] Scripts .bat validados.` exit 0; con `.bat` roto temporal (`(emsdk)`) → `[ERROR] ... abortando build.` exit 1. Bloque insertado pasa el escáner y preserva CRLF. Changelog #716 |
| 2026-08-01 | Migración :: → REM en bloques | ✅ | Comentarios `::` indentadas (dentro de bloques parenthesized) migradas a `REM` en `host/scripts/build_plugins.bat` (7 líneas) y `host/lint.bat` (2 líneas); cabeceras top-level conservadas como `::`. Evidencia empírica (bats mínimos, cmd.exe real): `::` en bloques **no** falló en casos simples (if/for/goto) — el footgun documentado es sutil (interacción con `goto` y re-parseo de labels), migración preventiva/buena práctica. Escáner: 0 issues repo completo, sin `::` indentadas restantes. Changelog #717 |
