/**
 * Diagnostic scanner: scans .bat files for the build_wasm.bat paren bug.
 *
 * The bug: an unescaped ')' inside a multi-line `if (...)`/`for ... do (...)`/`else (`
 * block closes the block early; trailing text on the same line then breaks cmd.exe
...
function collectBats(dir, out = []) {
  for (const entry of readdirSync(dir)) {
    if (entry === 'node_modules' || entry === 'JUCE' || entry === 'build' || entry === '.next' || entry === '.git' || entry === 'CMake') continue;
