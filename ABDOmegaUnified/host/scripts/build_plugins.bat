...
for %%f in ("%PLUGIN_SRC%\*.c" "%PLUGIN_SRC%\*.cpp") do (
    ...
    REM Compile to WASM using discovered Clang
    REM Added -fno-exceptions -fno-rtti for minimal C++ footprint
    ...
    if exist "!OUT_DIR!\!FILENAME!.wasm" (
        ...
        REM Era 7 - Extract Contract JSON automatically
        ...
        REM AOT Optimization if wamrc is present
        where wamrc >nul 2>&1
        if !ERRORLEVEL! EQU 0 (
